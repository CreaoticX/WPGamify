...
function cp_manage_form_submit() {
    ...
    $points = (int) filter_input(INPUT_POST, 'points');
    $uid = (int) filter_input(INPUT_POST, 'user_id');
    if ($points != '' && $uid != '') {
...
function cp_add_points_user_update() {
    ...
    if (!current_user_can('manage_options') || $id == '' || $points == '' || $description == '') {
        $response = json_encode(array('status' => 'failed'));
        echo $response;
        exit;
    }

    cp_points('addpoints', $id, $points, htmlentities($description));
    $response = json_encode(array(
        'status' => 'ok',
