...
if (cp_module_activated('youtube')) {
    ...
    function cp_module_youtube_config_process() {
        $module_youtube_points = (int) filter_input(INPUT_POST, 'cp_module_youtube_points');
        $cp_module_youtube_points = ($module_youtube_points < 0) ? 0 : $module_youtube_points;
    ...
    function cp_module_youtube_shortcode($atts) {
        ...
        $post_uuid = filter_input(INPUT_POST, 'uuid');
        $post_uuid = str_replace('__', '-', $post_uuid);
        ...
        if ($action == 'cp_youtube' && $post_uuid == $atts['id']) {
            ...
            if ((int) $wpdb->get_var("SELECT COUNT(*) FROM " . CP_DB . " WHERE `uid`=$uid AND `data`='$data' AND `type`='youtube'") == 0) {
                $wpgamify_points_core->wpg_add_points('youtube', $wpgamify_points_core->wpg_currentUser(), $points, $atts['id']);
            }
            exit();
        }
