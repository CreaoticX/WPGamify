...
if (cp_module_activated('donate')) {
    ...
    function cp_module_donate_scripts() {
        ...
        wp_register_script('impromptu', CP_PATH . 'modules/donate/jquery-impromptu.4.0.min.js', array('jquery'), '4.0');
    ...
    function cp_module_donate_do() {
        ...
        if (!is_user_logged_in()) {
            ...
            $wpgamify_points_core->wpg_points('donate_from', $user->ID, $points, serialize(array("from" => cp_currentUser(), "message" => $message)));
            $wpgamify_points_core->wpg_points('donate_to', cp_currentUser(), -$points, serialize(array("to" => $user->ID, "message" => $message)));
    ...
    function cp_admin_logs_desc_donate($type, $uid, $points, $data) {
        if ($type != 'donate_to' && $type != 'donate_from') {
            return;
        }
