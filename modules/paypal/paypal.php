...
if ($wpgamify_points_core->wpg_module_activated('paypal')) {
    ...
    function cp_modules_paypal_admin() {
        ...
        $cp_module_paypal_form_submit = filter_input(INPUT_POST, 'cp_module_paypal_form_submit');
        if ($cp_module_paypal_form_submit == 'Y') {

            update_option('cp_module_paypal_account', trim(filter_input(INPUT_POST, 'cp_module_paypal_account')));
            update_option('cp_module_paypal_sandbox', (bool) filter_input(INPUT_POST, 'cp_module_paypal_sandbox'));
            update_option('cp_module_paypal_currency', filter_input(INPUT_POST, 'cp_module_paypal_currency'));
            update_option('cp_module_paypal_item', trim(filter_input(INPUT_POST, 'cp_module_paypal_item')));
            $cp_module_paypal_cancel = trim(filter_input(INPUT_POST, 'cp_module_paypal_cancel'));
            if ($cp_module_paypal_cancel == '') {
                $cp_module_paypal_cancel = get_bloginfo('url') . '/?cp_module_paypal_return=0';
            }
            update_option('cp_module_paypal_cancel', $cp_module_paypal_cancel);
            $cp_module_paypal_thankyou = trim(filter_input(INPUT_POST, 'cp_module_paypal_thankyou'));
            if ($cp_module_paypal_thankyou == '') {
                $cp_module_paypal_thankyou = get_bloginfo('url') . '/?cp_module_paypal_return=1';
            }
            update_option('cp_module_paypal_thankyou', $cp_module_paypal_thankyou);
            $cp_module_paypal_price = (float) filter_input(INPUT_POST, 'cp_module_paypal_price');
            update_option('cp_module_paypal_price', ( $cp_module_paypal_price <= 0) ? 1 : $cp_module_paypal_price);
            $cp_module_paypal_min = (int) filter_input(INPUT_POST, 'cp_module_paypal_min');
            update_option('cp_module_paypal_min', ( $cp_module_paypal_min <= 0) ? 1 : $cp_module_paypal_min);
            update_option('cp_module_paypal_form', trim(stripslashes(filter_input(INPUT_POST, 'cp_module_paypal_form'))));
        ...
        function cp_module_paypal_currSel($curr) {
            if ($curr == get_option('cp_module_paypal_currency')) {
                echo 'selected';
            }
        }
        
        $cp_module_paypal_sandbox_checked = '';
        ...
        <div class="wrap">
            ...
            <form name="cp_module_paypal_form" method="post">
                ...
                <small><?php _e('Choose a preset', 'cp'); ?>: 
                    <a href="#" onclick="document.getElementById('cp_module_paypal_form').value = string1;
                                    return false;"><?php _e('Enter any amount', 'cp'); ?></a> |
                    <a href="#" onclick="document.getElementById('cp_module_paypal_form').value = string2;
                                    return false;"><?php _e('Select from a list', 'cp'); ?></a> |
                    <a href="#" onclick="document.getElementById('cp_module_paypal_form').value = string3;
                                    return false;"><?php _e('Single button with fixed points', 'cp'); ?></a> |
                    <a href="#" onclick="document.getElementById('cp_module_paypal_form').value = string4;
                                    return false;"><?php _e('Link', 'cp'); ?></a>
                </small>
        ...
        if (isset($_REQUEST['cp_module_paypal_pay']) && $_REQUEST['cp_module_paypal_pay'] != '') {
            ...
            ?>">
                    ...
                    <div id="container">
                        ...
                        <p><a href="#" onclick="document.paypal_form.submit();
                                            return false;">Click here to continue if you are not automatically redirected &raquo;</a></p> 
                    </div> 
                    <script type="text/javascript">
    ...
    function cp_module_paypal_ipn() {
        $fp = filter_input(INPUT_GET, 'cp_module_paypal_ipn');
        if (isset($fp) && $fp != '') {
            ...
            foreach ($_POST as $key => $value) {
                $filter_val = filter_input(INPUT_POST, $key);
                $filter_val = urlencode(stripslashes($filter_val));
                $req .= "&$key=$filter_val";
            }
