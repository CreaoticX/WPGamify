...
if ($wpgamify_points_core->wpg_module_activated('dailypoints')) {
    ...
    function cp_module_dailypoints_checkTimer() {
        ...
        if ($count != 0)
            return;
        $wpgamify_points_core->wpg_add_points('dailypoints', $uid, get_option('cp_module_dailypoints_points'), '');
    }
    ...
    function cp_module_dailypoints_config_process() {
        $cp_module_dailypoints_points = (int) filter_input(INPUT_POST, 'cp_module_dailypoints_points');
        $cp_module_dailypoints_time = (int) filter_input(INPUT_POST, 'cp_module_dailypoints_time');
        if ($cp_module_dailypoints_time < 0){
            $cp_module_dailypoints_time = 0;
        }
