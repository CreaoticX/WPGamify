...
if (cp_module_activated('customp')) {
    ...
    function cp_module_customp_box() {
        ...
        echo '<br /><input type="checkbox" id="cp_module_customp_enable" name="cp_module_customp_enable" value="1" size="25" ' . ((bool) (get_post_meta($post->ID, 'wpg_points_enable', 1)) ? 'checked="yes"' : '') . ' /> ';
        ...
        echo '<input type="text" id= "cp_module_customp_points" name="cp_module_customp_points" value="' . (int) get_post_meta($post->ID, 'wpg_points', 1) . '" size="25" /><br /><br />';
    }
    ...
    function cp_module_customp_save_postdata($post_id) {
        ...
        update_post_meta($post_id, 'wpg_points_enable', $cp_module_customp_enable);
        update_post_meta($post_id, 'wpg_points', $cp_module_customp_points);
    }
    ...
    function cp_module_customp_newComment($cid) {
        if (is_user_logged_in()) {
            ...
            $customp_enabled = (bool) get_post_meta($pid, 'wpg_points_enable', 1);
            $customp_points = (int) get_post_meta($pid, 'wpg_points', 1);
            if ($customp_enabled) {
