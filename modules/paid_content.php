...
if (cp_module_activated('pcontent')) {
    ...
    function cp_module_pcontent_config_process() {
        $cp_module_pcontent_default_points = (int) filter_input(INPUT_POST, 'cp_module_pcontent_default_points');
        $cp_module_pcontent_default = (bool) filter_input(INPUT_POST, 'cp_module_pcontent_default');
        $cp_module_pcontent_payauthor = (bool) filter_input(INPUT_POST, 'cp_module_pcontent_payauthor');
        $cp_module_pcontent_text_pay = filter_input(INPUT_POST, 'cp_module_pcontent_text_pay');
        $cp_module_pcontent_text_button = filter_input(INPUT_POST, 'cp_module_pcontent_text_button');
        $cp_module_pcontent_text_logout = filter_input(INPUT_POST, 'cp_module_pcontent_text_logout');
        $cp_module_pcontent_text_insufficient = filter_input(INPUT_POST, 'cp_module_pcontent_text_insufficient');
    ...
    function cp_module_pcontent_save_postdata($post_id) {
        ...
        $cp_module_pcontent_nonce = filter_input(INPUT_POST, 'cp_module_pcontent_nonce');
        if (!wp_verify_nonce($cp_module_pcontent_nonce, plugin_basename(__FILE__))) {
            return $post_id;
        }
        ...
        $cp_module_pcontent_points = (int) filter_input(INPUT_POST, 'cp_module_pcontent_points');
        if ($points < 1) {
            $points = 1;
        }
        
        $cp_module_pcontent_enable = (int) filter_input(INPUT_POST, 'cp_module_pcontent_enable');
        update_post_meta($post_id, 'cp_pcontent_points_enable', $cp_module_pcontent_enable);
    ...
    function cp_module_pcontent_buy() {
        $cp_module_pcontent_pay = filter_input(INPUT_POST, 'cp_module_pcontent_pay');
        if (!isset($cp_module_pcontent_pay))
            return;
        ...
        cp_points('pcontent', cp_currentUser(), -get_post_meta($cp_module_pcontent_pay, 'cp_pcontent_points', 1), $cp_module_pcontent_pay);
        if (get_option('cp_module_pcontent_payauthor')) {
            $post = get_post($cp_module_pcontent_pay);
            cp_points('pcontent_author', $post->post_author, get_post_meta($cp_module_pcontent_pay, 'cp_pcontent_points', 1), serialize(array($cp_module_pcontent_pay, cp_currentUser())));
        }
    }
